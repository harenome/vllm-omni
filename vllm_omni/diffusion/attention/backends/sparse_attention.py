...
"""SPARSE_ATTENTION: an in-tree dispatcher to external sparse attention kernels.

vllm-omni owns only this dispatcher (the ``SPARSE_ATTENTION`` slot in
``DiffusionAttentionBackendEnum``). The actual kernels live in external,
pip-installable packages that register a *function* via the
``vllm_omni.sparse_attn`` entry-point group::

    [project.entry-points."vllm_omni.sparse_attn"]
    dummy_sparse = "dummy_sparse_attn:attn_fn"

Function contract (device-agnostic, maskless self-attention)::
...
head_dim]``; the function returns the same layout. ``is_causal`` and
...
def resolve_sparse_attn_plugin(name: str | None) -> Callable[..., torch.Tensor]:
    """Resolve a sparse attention kernel function by name from entry points.
    ...
    if not name:
        raise ValueError(
            "SPARSE_ATTENTION requires a kernel name. Set it via the per-role "
...
class SparseAttentionBackend(AttentionBackend):
    ...
    ``accept_output_buffer=False``, ``supports_attention_mask()=False`` and the
    default ``supports_kv_cache_dtype()`` (no quant) all match the maskless,
    no-out-buffer sparse function contract. ``get_supported_head_sizes()==[]``
    means "any" — the external kernel owns its own head-size support.
    """
...
class _SparseAttentionImpl(AttentionImpl):
    def __init__(
        ...
        **extra_impl_args: Any,
        ...
        self._fn = resolve_sparse_attn_plugin(self._kernel_name)
    ...
    def forward(
        ...
        return self._fn(query, key, value, params, is_causal=self.causal, softmax_scale=self.softmax_scale)

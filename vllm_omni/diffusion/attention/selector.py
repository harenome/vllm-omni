...
def _route_sparse_plugin_shorthand(spec: AttentionSpec) -> AttentionSpec:
    """Route a sparse-kernel shorthand (``backend=<plugin>``) to SPARSE_ATTENTION.
    ...
    ``SPARSE_ATTENTION`` dispatcher with the plugin name + the original ``extra``
    ...
    Built-in names (the dense backends *and* SPARSE_ATTENTION itself) are
    ...
    return _AttentionSpec(
        backend=DiffusionAttentionBackendEnum.SPARSE_ATTENTION.name,

...
| `SPARSE_ATTENTION` | In-tree dispatcher to **external** sparse-attention kernels registered via the `vllm_omni.sparse_attn` entry-point group (no kernels are vendored). Opt-in per role; maskless self-attention only. See [Sparse attention (plugin)](#sparse-attention-plugin). |
...
`SPARSE_ATTENTION` is an in-tree *dispatcher*: vLLM-Omni owns only the slot, while
the actual sparse kernels ship as **external, pip-installable packages**. A kernel
package registers a function via the `vllm_omni.sparse_attn` entry-point group
...
and the function returns the same layout. `is_causal` and `softmax_scale` are the
...
vllm-omni serve Wan-AI/Wan2.2-T2V-A14B-Diffusers \
    --diffusion-attention-config '{"per_role":{"self":{"backend":"SPARSE_ATTENTION","extra":{"backend":"dummy_sparse","params":{"topk":0.3}}}}}'
```

...
"""Tests for the SPARSE_ATTENTION dispatcher backend.
...
class TestSparseAttentionBackend:
    ...
    def test_registered_in_enum(self):
        assert DiffusionAttentionBackendEnum.SPARSE_ATTENTION.get_class() is SparseAttentionBackend
...
class TestPluginShorthandRouting:
    """backend=<plugin> routes to the SPARSE_ATTENTION dispatcher."""
    ...
    def test_shorthand_routed_to_dispatcher(self):
        ...
        assert out.backend == "SPARSE_ATTENTION"
    ...
    def test_explicit_sparse_attention_unchanged(self):
        spec = AttentionSpec(backend="SPARSE_ATTENTION", extra={"backend": "dummy_sparse", "params": {"topk": 0.5}})
